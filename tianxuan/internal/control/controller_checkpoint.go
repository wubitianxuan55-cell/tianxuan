...
// resolveBranch resolves a human-supplied ref (id prefix, name, path) to a branch.
...
func (c *Controller) summarizeAt(ctx context.Context, turn int, from bool) error {
	...
	// Since V5.0, LLM-based summarisation has been replaced by automatic
	// truncation in the run loop (≥500K tokens → three-tier compression).
	// The summarise command is retained for compatibility but performs no
	// conversation restructuring — the built-in truncation handles context
	// pressure automatically.
	c.notice("summarize: automatic truncation handles context compression — no manual summarise needed")
	return nil
}
