...
func TestCacheHitSurvivesTooSmallWindow(t *testing.T) {
	...
	if collapses > 4 {
		t.Errorf("compaction cratered the cache %d times; the stuck guard should cap it at ≤4", collapses)
	}
	// tianxuan V5.0: no auto-compaction-paused guard — the simple truncation
	// strategy naturally limits itself (can't truncate fewer than minRecentKeep).
	if paused {
		t.Log("auto-compaction paused notice emitted (not expected in tianxuan)")
	}
	...
	if n := len(sink.usages); n >= 6 {
		if tail := tailAverage(usageRates(sink.usages), 5); tail < 80 {
