...
type Messages struct {
	...
	StepScaffold    string // step 1 desc — tianxuan setup
	StepSetKey      string // step 2 command label

	// `tianxuan init` — points to the in-session /init skill + setup
	InitHint       string
	StepSetKeyHint string // step 2 desc — env var hint
	StepChatDesc   string // tianxuan chat step desc
	StepRunDesc    string // tianxuan run step desc
	HelpFooter     string // dim footer linking to tianxuan help
	...
	WroteFileFmt          string // "Wrote %s" — used for tianxuan.toml and .env both
	...
	UsageRunHint      string // "usage: tianxuan run [--model NAME] <task>"
...
//
// Priority: override > TIANXUAN_LANG > LC_ALL > LC_MESSAGES > LANG > "en".
...
func envCandidates() []string {
	keys := []string{"TIANXUAN_LANG", "LC_ALL", "LC_MESSAGES", "LANG"}
