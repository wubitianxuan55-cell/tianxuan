...
func resetTree(repo string) {
	...
	_ = exec.Command("git", "-C", repo, "clean", "-fd", "-e", "tianxuan.toml").Run()
}
...
func renderDiff(r diffReport) string {
	...
	fmt.Fprintf(&b, "## 🤖 Tianxuan e2e — diff test-gen\n\n")
...
// repoRelFromModulePath turns "tianxuan/internal/agent/foo.go" into
