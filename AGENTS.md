...
- **编码铁律**（自动生效）：
  - 🔴 **中文思考与输出**：所有分析、推理、回复必须使用简体中文。代码、标识符、文件路径、Shell 命令和技术术语保持原文。
  - 🔴 **设计优先**：编码前必须完成需求探索和方案设计。用 `ask`/`explore` 工具自行设计后编码，获得批准前不动代码。
  ...
  - 🔴 **必须用 `ask` 工具提问**：需用户决策时必须调 `ask` 工具弹选择卡。严禁纯文本提问——纯文本 = 轮次终止 = 重新规划，浪费一整轮。
...
### 1. 设计优先法则
用 `ask`/`explore` 工具调查代码 → 提出方案 → 获用户批准后编码。即使是"简单"任务也必须走流程——简单任务的设计可以很短，但不能跳过。
...
| "我需要先了解上下文" | 技能/工具告诉你如何了解。用 `ask`/`explore` 调查代码再行动。 |
