...
type mdRenderer struct {
	...
	width    int
	diffFold bool // when true, collapses context lines in diff code blocks
}
...
func (r *mdRenderer) renderFenced(buf *strings.Builder, n ast.Node, src []byte, indent int) {
	// Detect diff info string for folding support.
	isDiff := false
	if fcb, ok := n.(*ast.FencedCodeBlock); ok && r.diffFold {
