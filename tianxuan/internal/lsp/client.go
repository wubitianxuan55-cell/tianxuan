...
func (c *client) completion(ctx context.Context, uri string, pos Position) (json.RawMessage, error) {
	return c.query(ctx, "textDocument/completion", uri, pos)
}

func (c *client) rename(ctx context.Context, uri string, pos Position, newName string) (json.RawMessage, error) {
	return c.callRetry(ctx, "textDocument/rename", map[string]any{
		"textDocument": map[string]any{"uri": uri},
		"position":     pos,
		"newName":      newName,
	})
