...
// buildExamplePlugin compiles cmd/tianxuan-plugin-example into a temp binary and
...
func buildExamplePlugin(t *testing.T) string {
	t.Helper()
	bin := filepath.Join(t.TempDir(), "tianxuan-plugin-example")
	if runtime.GOOS == "windows" {
		bin += ".exe"
	}
	out, err := exec.Command("go", "build", "-o", bin, "tianxuan/cmd/tianxuan-plugin-example").CombinedOutput()
	if err != nil {
