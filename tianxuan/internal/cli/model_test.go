...
// provider/model refs (built-in defaults when no tianxuan.toml is present), and
// only those whose provider API key is set.
func TestModelRefsFromConfig(t *testing.T) {
	t.Chdir(t.TempDir()) // no tianxuan.toml → built-in default providers
...
func TestModelRefsSkipsUnconfigured(t *testing.T) {
	...
	for _, ref := range modelRefs() {
		// XAI uses OAuth — may be auto-configured without env vars
