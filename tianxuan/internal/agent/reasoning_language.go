...
// withTurnPreferences wraps user input with transient language preference
// blocks. These blocks are injected at the front of each user turn so they
// stay out of the cache-stable system prompt — changing the language between
// turns does not churn the DeepSeek prefix cache.
// (Design adopted from DeepSeek-Reasonix-V1.12)
func (a *AgentRunner) withTurnPreferences(input string) string {
	if a == nil {
		return input
	}
	responseLang := "auto"
...
// SetResponseLanguage sets the runtime final-answer language preference.
// Empty string resets to "auto" (follow user language).
func (a *AgentRunner) SetResponseLanguage(lang string) {
	a.responseLanguage.Store(NormalizeResponseLanguage(lang))
}

// SetReasoningLanguage sets the runtime visible-reasoning language preference.
// Empty string resets to "auto".
func (a *AgentRunner) SetReasoningLanguage(lang string) {
