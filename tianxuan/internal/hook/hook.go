...
// (.tianxuan/settings.json, only when the project is trusted) and a global
// (~/.tianxuan/settings.json) file. A hook's exit
...
const (
	SettingsDirname  = ".tianxuan"
	SettingsFilename = "settings.json"
)

// GlobalSettingsPath is ~/.tianxuan/settings.json (homeDir overrides ~).
func GlobalSettingsPath(homeDir string) string {
	return filepath.Join(home(homeDir), SettingsDirname, SettingsFilename)
}

// ProjectSettingsPath is <root>/.tianxuan/settings.json.
