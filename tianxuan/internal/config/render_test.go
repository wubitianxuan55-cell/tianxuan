...
func TestRenderTOMLRoundTrips(t *testing.T) {
	...
	orig.Plugins = []PluginEntry{
		{Name: "example", Command: "tianxuan-plugin-example"},
	...
	if got.Agent.AutoPlan != orig.Agent.AutoPlan {
		t.Errorf("auto_plan = %q, want %q", got.Agent.AutoPlan, orig.Agent.AutoPlan)
	}
