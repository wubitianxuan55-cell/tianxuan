...
var English = Messages{
	...
	SetKeyHint:      "Set your API key, then run `tianxuan chat`.",
	ConfigLabel:     "config",
	ModelsLabel:     "models",
	...
	StepScaffold:    "scaffold tianxuan.toml",
	StepSetKey:      "set API key",

	InitHint:       "Project memory (AGENTS.md) is generated in-session: run `tianxuan chat`, then `/init` — the model analyzes the codebase and writes it. For configuration, use `tianxuan setup`.",
	StepSetKeyHint: "export DEEPSEEK_API_KEY=… or add to .env",
	StepChatDesc:   "interactive session",
	StepRunDesc:    "one-shot task",
	HelpFooter:     "tianxuan help · all commands",

	ChatTip:           "Context is kept across turns. Type 'exit' or Ctrl-D to quit.",
	TurnCancelled:     "cancelled — back to prompt",
	NoSessionToResume: "no saved session to resume — start a new one with `tianxuan chat`",
	...
	OutputStyleHint:   "set agent.output_style in tianxuan.toml to apply one (takes effect next session)",
	...
	SlashHelp:          "commands: /compact · /new · /rename · /diff-fold · /resume · /rewind · /tree · /branch · /switch · /todo · /verbose · /model (switch model) · /mcp · /skill · /hooks · /paste-image · /memory · /quit · /help · plus skills (/init, /explore, …)",
	SlashPromptEmpty:   "the MCP prompt returned no content to send",
	SlashMCPNone:       "no MCP servers configured — add a [[plugins]] entry in tianxuan.toml",
	...
	CmdRename:       "rename session",
	CmdDiffFold:     "toggle diff folding",
	...
	ListHooksNone:       "hooks: none active — configure in .tianxuan/settings.json (project, after trust) or ~/.tianxuan/settings.json (global)",
	ListMcpHeader:       "mcp servers",
	ListMcpNone:         "mcp: no servers connected — add one in tianxuan.toml ([[plugins]]) or a project .mcp.json",

	SelectProvidersLabel:  "Select providers to enable",
	EnterAPIKeysHeader:    "Enter API keys (Enter to skip and set later in .env):",
	MissingKeyIntro:       "tianxuan.toml is ready — just an API key away.",
	WroteFileFmt:          "Wrote %s",
	SetupComplete:         "Setup complete.",
	SetupCancelled:        "setup cancelled.",
	TryHintFmt:            "Try: %s",
	NextHint:              "Next: set your API key (export DEEPSEEK_API_KEY=... or add to .env), then run `tianxuan run \"your task\"`.",
	...
	UsageRunHint:      "usage: tianxuan run [--model NAME] <task>",
	ErrorPrefix:       "error:",
	...
	UsageBody: `tianxuan — a config- and plugin-driven coding agent (multi-model)

Usage:
  tianxuan chat [--model NAME] [-c|--continue] [--resume]   interactive session (multi-turn; -c resumes the latest, --resume picks one)
  tianxuan run  [--model NAME] [--max-steps N] <task>   run one task and exit
  tianxuan serve [--model NAME] [--addr HOST:PORT]      serve the session over HTTP+SSE (browser client at /)
  tianxuan setup [path]                                  interactive config wizard; writes tianxuan.toml (+ .env)
  tianxuan mcp <add|remove|list>                         manage MCP servers in tianxuan.toml
  tianxuan version
  tianxuan update [--repo owner/repo] [--tag vX.Y.Z] [--check]   self-update from GitHub
  tianxuan help

Examples:
  tianxuan chat
  tianxuan chat --continue
  tianxuan run "implement the TODOs in main.go"
  tianxuan run --model mimo-pro "add unit tests for this function"
  echo "explain this code" | tianxuan run

Configuration:
  Resolution: flag > ./tianxuan.toml > ~/.config/tianxuan/config.toml > built-in defaults
  Secrets come from the environment via api_key_env (e.g. DEEPSEEK_API_KEY).
  Run 'tianxuan setup' to scaffold a config; see docs/SPEC.md.
`,
}
