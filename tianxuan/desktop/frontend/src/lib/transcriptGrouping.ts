...
function stripAttachmentRefs(text: string): string {
  return text.replace(/\[\[reasonix-attach:[^\]]+\]\]/g, "");
}
...
export function questionTurnsById(questions: QuestionAnchor[]): Map<string, number> {
  const hasCheckpointTurns = questions.some((q) => q.checkpointTurn != null);
  const turns = new Map<string, number>();
  for (const q of questions) {
    if (q.checkpointTurn != null) turns.set(q.id, q.checkpointTurn);
    else if (!hasCheckpointTurns) turns.set(q.id, q.turn);
  }
  return turns;
}
...
export function buildTurnGroups(items: Item[]): TurnGroup[] {
  ...
  for (let i = 0; i < items.length; i += 1) {
    if (items[i].kind === "user") {
      if (start >= 0) groups[groups.length - 1].endIdx = i;
      start = i;
      groups.push({ userItem: items[i], assistantPreview: "", toolCount: 0, startIdx: i, endIdx: items.length });
    } else if (start >= 0 && groups.length > 0) {
      const group = groups[groups.length - 1];
      const item = items[i];
      if (item.kind === "assistant" && !item.streaming) {
        const previewText = item.text?.trim() || "";
        if (previewText) group.assistantPreview = warmUserPreview(previewText);
      }
      if (item.kind === "tool" && !("parentId" in item && item.parentId)) {
        group.toolCount += 1;
      }
    }
  }
...
export function warmPagination({ turnCount, hotTurns, pageSize, coldPage }: {
  turnCount: number; hotTurns: number; pageSize: number; coldPage: number;
  ...
  return { warmStartTurn: warmEndTurn - shownWarmCount, warmEndTurn, coldTurnCount: warmEndTurn - shownWarmCount };
}

export function warmColdPageForTurn({ turn, turnCount, hotTurns, pageSize }: {
  turn: number; turnCount: number; hotTurns: number; pageSize: number;
}): number {
  const safeTurnCount = Math.max(0, turnCount);
  const safeHotTurns = Math.max(0, hotTurns);
  const warmEndTurn = Math.max(0, safeTurnCount - Math.min(safeTurnCount, safeHotTurns));
  ...
  return Math.max(0, Math.ceil((warmEndTurn - Math.max(0, Math.floor(turn))) / safePageSize) - 1);
}
