...
// mcp.go holds the MCP server-management surface shared by the `tianxuan mcp`
...
// mcpCommand implements `tianxuan mcp <add|remove|list>`. It edits config only
...
func mcpList() int {
	...
	if cfg.Codegraph.Enabled {
		if bin, ok := codegraph.Resolve(cfg.Codegraph.Path); ok {
			...
			fmt.Printf("%-16s (built-in, not installed)  run `tianxuan codegraph install` (or it auto-installs on first use)\n", "codegraph")
		}
		listed++
	}
...
func mcpRemoveCLI(args []string) int {
	if len(args) == 0 {
		fmt.Fprintln(os.Stderr, "usage: tianxuan mcp remove <name>")
		return 2
	}
	name := args[0]
	cfg, err := config.Load()
...
func mcpUsage() {
	fmt.Println(`Manage MCP servers (persisted to tianxuan.toml).

Usage:
  tianxuan mcp list
  tianxuan mcp add <name> <command> [args...]        stdio server
  tianxuan mcp add <name> --http <url> [--header K=V] remote (Streamable HTTP)
  tianxuan mcp add <name> --sse  <url>               remote (legacy SSE)
  tianxuan mcp remove <name>
...
Examples:
  tianxuan mcp add fs npx -y @modelcontextprotocol/server-filesystem .
  tianxuan mcp add stripe --http https://mcp.stripe.com --header "Authorization=Bearer $STRIPE_KEY"
