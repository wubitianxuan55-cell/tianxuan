...
// Trust gates project hooks. A project's .tianxuan/settings.json can run
...
// root. The trust flag lives in user-global state (~/.tianxuan/trust.json),
// NOT in the project file itself — an attacker controls the latter. Global
// hooks (~/.tianxuan/settings.json) are the user's own and always run.

// TrustFilename is the user-global trust store under ~/.tianxuan.
...
// TrustPath is ~/.tianxuan/trust.json (homeDir overrides ~).
