...
- **慢测试标记**：16 个外部依赖类测试（真实 npx 执行、live 网络 API、真实
  进程树、5000 次并发压力）加 `testing.Short()` 守卫，`-short` 下跳过；
  确定性行为测试（jobs 等待语义等）保留在快速门禁
- **CI 门禁提速**：ci.yml test job 改 `go test -short ./...`，Windows 本机
  全量 48s → 23s（builtin 包 40s → 13.6s）
- **夜间全量覆盖**：新增 nightly job（cron 每日 20:00 UTC + workflow_dispatch
...
- `boot.go` 1031 行拆至 807 行：子代理引导辅助（fork 上下文渲染、子代理
  模型解析、技能模板前缀、compact 工具集）→ `subagent.go`（255 行）
- 23 个函数体逐字节验证零改动（纯移动）
