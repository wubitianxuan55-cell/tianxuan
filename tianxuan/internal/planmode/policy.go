// Package planmode is the single plan-mode tool-safety policy shared by the
// permission auto-approve gate and the read-only planning stage.
...
import (
	...
	"tianxuan/internal/shellsafe"
)

// Marker is the model-facing plan-mode instruction block. V10.134: ends with
// the ask-approval workflow used by single-model auto-plan — the model must
// request explicit approval via ask; the host switches off the read-only gate
// when the user picks "提交执行", so the same turn can proceed to execution.
const Marker = "[Plan mode — planning only. You may research the codebase and web with read-only tools (read_file, grep, glob, ls, web_search, web_fetch), ask clarifying questions with ask, and maintain planning state with todo_write. You must not write files, run unsafe shell commands, install capabilities, mutate memory, dispatch sub-agents, control long-lived processes, or mark execution steps complete. Before planning, if a decision that is genuinely the user's — tech stack, an ambiguous requirement, scope, an irreversible choice — would materially shape the plan and you can't settle it from the codebase or a sensible default, use the ask tool to clarify it first; otherwise pick the obvious default and state the assumption in the plan instead of asking. Then present a LAYERED plan as your reply and stop. Structure the plan as a two-level markdown list so it becomes a layered task list: each PHASE is a top-level numbered list item (a coherent milestone, e.g. \"1. Add the config loader\"), and each phase's concrete, verifiable sub-steps are bullets indented beneath it (e.g. \"   - parse the TOML into Config\"). Use plain numbered list items for phases — do NOT write phases as markdown headings (##, ###) — so both levels parse. Keep phases few (about 2-6). The user will be asked to approve before any changes are made. AFTER presenting the plan, immediately call the ask tool with a single approval question (header: '计划批准', question: '是否提交执行以下计划？', options in this exact order: '提交执行' / '按意见修改' / '取消'). Wait for the answer: '提交执行' lets the host switch you to execution mode so you can proceed in the same turn; '按意见修改' keeps you in plan mode — revise the plan and ask again; '取消' ends the turn with no changes.]"

// PlanSafety is a tool's self-reported stance on running during the planning
// phase, surfaced via PlanModeClassifier.
type PlanSafety int
...
// ReadOnlyCommandTrust is a candidate shell command prefix for user trust.
...
// Class is the plan-mode bucket a tool falls into.
type Class int
...
// Classify reports the plan-mode bucket for a tool.
