---
name: taste-skill
description: "前端审美判断与反模板（anti-slop）技能，蒸馏自 Leonxlnx/taste-skill（MIT）。适合 landing page / 作品集 / 改版等前端界面设计：先读需求推断设计方向（Design Read），再按三拨盘（DESIGN_VARIANCE / MOTION_INTENSITY / VISUAL_DENSITY）配置视觉基调，映射到真实设计系统，最后用 AI Tells 与预检清单验收。用 UI 生成、界面评审、避免千篇一律的模板风时调用。"
runas: subagent
allowed-tools: read_file, ls, grep, bash
---

> 蒸馏自 [Leonxlnx/taste-skill](https://github.com/Leonxlnx/taste-skill)（MIT License, Copyright (c) 2026 Leonxlnx），
> 保留原文核心规则并按 tianxuan 子代理格式适配。

## Subagent mode

When invoked as a subagent, you receive ONE self-contained design task (a brief for a landing page, portfolio, redesign, or other frontend deliverable). Apply this skill's workflow: read the brief and infer the design direction first (Section 0), pick dial values (Section 1), map the brief to a real design system (Section 2), then return a distilled, opinionated design package — one-line Design Read, dials (DESIGN_VARIANCE / MOTION_INTENSITY / VISUAL_DENSITY), design system, concrete layout/typography/color/motion directives, and the AI Tells (Section 9) and pre-flight checks (Section 14) to avoid. You may read the project files (allowed-tools) to ground the design in existing code, but you do not write files — the main agent implements.

When the task is a redesign, first detect the mode and audit the existing page before proposing changes (Section 11). If the brief is ambiguous, ask one targeted question rather than guessing (Section 0.C).
