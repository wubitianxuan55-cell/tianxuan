...
func TestBuiltinsRegistered(t *testing.T) {
	want := []string{"bash", "doctor", "edit_file", "glob", "grep", "ls", "multi_edit", "read_file", "web_fetch", "web_search", "write_file"}
...
func TestBuiltinReadOnlyClassification(t *testing.T) {
	readOnly := map[string]bool{
		"read_file": true, "ls": true, "glob": true, "grep": true, "doctor": true, "web_fetch": true, "web_search": true,
...
func TestEditFile(t *testing.T) {
	...
	runTool(t, editFile{}, map[string]any{"path": f, "old_string": "world", "new_string": "tianxuan"})
	if b, _ := os.ReadFile(f); string(b) != "hello tianxuan\n" {
...
func TestMultiEdit(t *testing.T) {
	...
	out := runTool(t, multiEdit{}, map[string]any{
		...
		"edits": []map[string]any{
			...
			{"old_string": "old", "new_string": "tianxuan", "replace_all": true},
		},
	})
