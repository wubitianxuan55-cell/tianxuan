# Tianxuan Desktop (Wails shell)

A native desktop window around the Tianxuan Go kernel. The same
...
`desktop/` is its own Go module (`module tianxuan/desktop`, `replace tianxuan =>
...
`tianxuan/` so it can still import the `tianxuan/internal/*` kernel.
...
```sh
cd desktop
wails build          # → build/bin/Tianxuan(.app/.exe)
```
...
- **macOS** — Gatekeeper may report the app "is damaged" or is from an
  ...
  ```sh
  xattr -dr com.apple.quarantine /Applications/Tianxuan.app
  ```
