...
export function ToolCard({ item, subcalls }: { item: ToolItem; subcalls?: ToolItem[] }) {
  ...
  return (
    <div className={`my-0.5 rounded-lg overflow-hidden border transition-colors duration-300 ${
      ...
      item.status === "running" ? "border-accent/30 bg-accent/[0.03]" :
      item.status === "stopped" ? "border-border-soft opacity-70" :
      "border-border-soft"
    } ${quiet ? "border-transparent bg-transparent" : ""}`}>
      <div
        className={`flex items-center gap-2 px-2.5 py-1.5 text-fg-dim text-[12.5px] select-none ${
          expandable ? "cursor-pointer hover:bg-bg-soft" : ""
        }`}
        onClick={expandable ? () => setOpen((v) => !v) : undefined}
      >
        {expandable ? (
          <ChevronRight className={`shrink-0 transition-transform duration-200 ${effectiveOpen ? "rotate-90" : ""}`} size={13} />
        ) : (
          <span className="w-[13px] shrink-0" />
        )}
        <Icon className={`shrink-0 ${item.status === "error" ? "text-err" : item.status === "running" ? "text-accent animate-pulse" : "text-fg-faint"}`} size={14} />
      ...
      <div className={`grid transition-all duration-300 ease-in-out ${
        ...
        <div className="overflow-hidden min-h-0">
          ...
          {item.error && <div className="px-2.5 py-1.5 text-err text-[12px] leading-relaxed border-t border-err/20">{item.error}</div>}
        </div>
      </div>
    </div>
  );
