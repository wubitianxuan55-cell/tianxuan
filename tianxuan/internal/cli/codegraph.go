...
// codegraphCommand backs `tianxuan codegraph` — managing the CodeGraph
// code-intelligence runtime that tianxuan otherwise fetches lazily on first use.
...
func codegraphStatus() int {
	...
	if p, ok := codegraph.Resolve(cfg.Codegraph.Path); ok {
		...
		fmt.Printf("%-13s %s\n", "resolved:", "(not installed — run `tianxuan codegraph install`)")
	}
	return 0
}

func codegraphUsage() {
	fmt.Print(`tianxuan codegraph — manage the CodeGraph code-intelligence runtime

Usage:
  tianxuan codegraph install   download + cache the runtime for this platform
  tianxuan codegraph status    show config, cache dir, and resolved launcher
