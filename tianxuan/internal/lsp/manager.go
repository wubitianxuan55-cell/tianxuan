...
// on PATH; nothing here ships with tianxuan. Extensions drive file routing, so a
...
func (m *Manager) Completion(ctx context.Context, file string, line int, symbol string) (string, error) {
	c, uri, pos, err := m.prepare(ctx, file, line, symbol)
	if err != nil {
		return "", err
	}
	raw, err := c.completion(ctx, uri, pos)
	if err != nil {
		return indexingOr(err)
	}
	return formatCompletions(raw), nil
}

func (m *Manager) Rename(ctx context.Context, file string, line int, symbol string, newName string) (string, error) {
	c, uri, pos, err := m.prepare(ctx, file, line, symbol)
	if err != nil {
		return "", err
	}
	raw, err := c.rename(ctx, uri, pos, newName)
	if err != nil {
		return indexingOr(err)
	}
